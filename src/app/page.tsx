...
export default async function HomePage() {
  ...
  const paidMap = new Map(
    paidAgg.map((c) => [c.itemId, Number(c._sum.paidUsed ?? 0)])
  );
  ...
  const winnersMap = new Map(
    winnerCounts.map((w) => [w.itemId, w._count._all])
  );
  ...
  return (
    <main className="flex min-h-[calc(100vh-172px)] flex-col gap-2.5 xl:min-h-[calc(100vh-182px)] xl:gap-3">
      <WelcomeModal />
      ...
      <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500 sm:text-[11px]">
        Logged in as{" "}
        <span className="font-bold normal-case tracking-normal text-slate-900">
          {user.email}
        </span>
      </div>

      <div className="grid flex-1 auto-rows-fr grid-cols-1 gap-3 sm:grid-cols-2 lg:min-h-0 lg:grid-cols-3 xl:gap-4 2xl:gap-5">
        {refreshed.map((it) => {
