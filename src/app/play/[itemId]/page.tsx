...
export default async function PlayPage({ params }: { params: { itemId: string } }) {
  ...
  try {
    ...
    return (
      <main className="mx-auto max-w-[900px] px-4 py-1.5 sm:px-5 lg:px-6">
        <div className="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <h1 className="mt-1 text-lg font-black text-slate-950">{item.title}</h1>
            </div>

            <div className="flex flex-wrap gap-2">
              <Link
                href={`/item/${itemId}/leaderboard`}
                className="rounded-2xl border border-slate-300 bg-white px-3.5 py-2 text-sm font-bold text-slate-900"
              >
                Leaderboard
              </Link>
              <Link
