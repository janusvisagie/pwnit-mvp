...
export default async function ItemLeaderboardPage({ params }: { params: { id: string } }) {
  ...
  const best = await prisma.attempt.groupBy({
    ...
    where: {
      itemId,
      dayKey,
      // Exclude invalid attempts (e.g. memory incorrect) from leaderboard scoring
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2 text-sm text-slate-600">
            <span>
              Live leaderboard • <span className="font-semibold text-slate-900">{dayKey}</span>
            </span>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <div className="text-xs text-slate-600">
            Lower ms is better • Players today: <span className="font-semibold text-slate-900">{totalPlayers}</span>
          </div>
        </div>
        ...
        {rows.length === 0 ? (
          <div className="px-4 py-4 text-sm text-slate-600">No scores yet today.</div>
        ) : (
          rows.map((r) => (
            <div
              key={r.userId}
