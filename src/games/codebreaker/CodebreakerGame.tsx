    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black text-slate-950">Codebreaker</h3>
        </div>
      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-2 flex gap-2">
          {Array.from({ length: CODE_LENGTH }, (_, index) => (
            <div
              key={index}
              className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-300 bg-white text-lg font-black text-slate-900"
            >
              {guess[index] ?? "•"}
            </div>
          ))}
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-6">
        {DIGIT_POOL.map((digit) => (
          <button
            ...
            className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-base font-black text-slate-900 transition hover:-translate-y-0.5 hover:border-slate-300 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
          >
            {digit}
          </button>
        ))}
      ...
      <div className="mt-4 space-y-2">
        {rows.map((row, index) => (
          <div key={`${row.value.join("")}-${index}`} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
