...
export default function ClueLadderGame({
  ...
  async function resolveLevel(selectedId: string | null, timedOut = false) {
    ...
    if (verifiedMode) {
      ...
      try {
        ...
        if (data.finished) {
          ...
          setMessage(
            nextStats.status === "TIMED_OUT"
              ...
              : "Wrong choice. This competitive run is over.",
          );
        ...
        setMessage(`Correct. Level ${Number(data.nextChallenge?.level || level + 1)} starts with clue 1.`);
  ...
  async function revealNext() {
    ...
    if (!verifiedMode) {
      ...
      setMessage(nextShown.length >= totalClues ? "All clues unlocked. Pick your answer." : "One more clue unlocked.");
      return;
    }
    ...
    try {
      ...
      setRunStats((prev) => ({ ...prev, totalRevealCount: prev.totalRevealCount + 1 }));
      setMessage(data.exhausted ? "All clues unlocked. Pick your answer." : "One more clue unlocked.");
    } catch (error: any) {
    ...
    <div className="mx-auto max-w-xl rounded-3xl border border-slate-200 bg-white p-3 shadow-sm sm:p-3.5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div className="mt-1 text-sm font-black text-slate-950">Use as few clues as possible, then answer fast.</div>
        </div>
        ...
        {Array.from({ length: totalClues }, (_, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-1 leading-5">{clue ? clue : `Locked clue ${index + 1}`}</div>
            </div>
          );
        })}
      </div>
