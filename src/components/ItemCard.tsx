...
export function ItemCard({ item }: { item: ItemCardModel }) {
  ...
  return (
    ...
    >
      <div className="relative flex min-h-[100px] items-center justify-center overflow-hidden bg-gradient-to-b from-slate-50 to-white p-2 sm:min-h-[108px] md:min-h-[114px]">
        <ProductImage
          primarySrc={primaryImage}
          fallbackSrc={fallbackImage}
          alt={item.title}
          className="flex items-center justify-center"
          imgClassName="max-h-[76px] max-w-full object-contain transition duration-300 group-hover:scale-[1.03] sm:max-h-[84px] md:max-h-[90px]"
        />
      ...
      <div className="flex min-h-0 flex-1 flex-col justify-between gap-1 p-2.5">
        <div className="min-w-0">
          <h3 className="line-clamp-2 text-[13px] font-extrabold leading-tight text-slate-900 sm:text-[14px] md:line-clamp-1">
            {item.title}
          </h3>
          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] leading-tight text-slate-600">
            {gLabel ? (
