...
export function ItemCard({ item }: { item: ItemCardModel }) {
  ...
  const primaryImage = item.imageUrl ?? product?.imageUrl ?? fallbackImage;
  ...
  return (
    <Link
      href={href}
      className="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.18)]"
    >
      {isClosed ? (
        <div className="pointer-events-none absolute left-[-18%] top-[46%] z-30 w-[136%] -translate-y-1/2 -rotate-12 bg-slate-950/96 px-4 py-2 text-center text-[10px] font-extrabold tracking-[0.18em] text-white shadow-xl sm:text-xs">
          PRIZE WON • CLOSED
        </div>
      ) : null}

      <div className="relative flex min-h-[190px] items-center justify-center bg-gradient-to-b from-slate-50 to-white p-6">
        <ProductImage
          primarySrc={primaryImage}
          fallbackSrc={fallbackImage}
          alt={item.title}
          className="flex w-full items-center justify-center"
          imgClassName="max-h-[138px] max-w-full object-contain transition duration-300 group-hover:scale-[1.03]"
        />

        <div className="absolute left-4 top-4 rounded-full bg-white/95 px-3 py-1.5 text-xs font-extrabold text-slate-900 shadow ring-1 ring-slate-200">
          {formatZAR(item.prizeValueZAR)}
        </div>

        <div className={`absolute right-4 top-4 rounded-full px-3 py-1.5 text-xs font-extrabold shadow-sm ${statusTone}`}>
          {statusText}
        </div>

        {hot ? (
          <div className="animate-pwnit-pulse-soft absolute bottom-4 left-4 rounded-full bg-amber-300 px-3 py-1.5 text-xs font-extrabold text-slate-900 shadow-sm">
            Hot
          </div>
        ) : null}
      ...
      <div className="flex flex-1 flex-col justify-between gap-3 p-5">
        <div>
          <h3 className="line-clamp-2 text-lg font-extrabold leading-tight text-slate-900">{item.title}</h3>
        ...
        {!isClosed ? (
          ...
          <div className="text-xs text-slate-500">View results or buy the prize if you didn’t win.</div>
        )}
      </div>
    </Link>
  );
