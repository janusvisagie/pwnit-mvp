...
if (-not $config.local.databaseUrl) {
  throw "db-targets.json does not contain local.databaseUrl"
}
...
$db = [string]$config.local.databaseUrl
$content = @(
  "DATABASE_URL=\"$db\""
  "POSTGRES_URL=\"$db\""
...
Write-Host "Switched .env and .env.local to LOCAL database."
if (-not $NoBackup) {
